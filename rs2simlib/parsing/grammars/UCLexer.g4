...
@lexer::header {
    import { Token, IntStream, LexerNoViableAltException } from 'antlr4ng';

    // TODO: Create a map of keyword tokens?
}

@lexer::members {
	public static readonly MACRO_CHANNEL = Token.MIN_USER_CHANNEL_VALUE; // 2
	public static readonly MACRO_HIDDEN_CHANNEL = Token.MIN_USER_CHANNEL_VALUE + 1; // 3
	public static readonly COMMENTS_CHANNEL = Token.MIN_USER_CHANNEL_VALUE + 2; // 4
...
LINE_COMMENT
    : '//' ~[\r\n]*
    -> channel(COMMENTS) //, pushMode(LINE_COMMENT_MODE)
    ;

BLOCK_COMMENT
    : '/*' (BLOCK_COMMENT|.)*? '*/'
    -> channel(COMMENTS) //, pushMode(BLOCK_COMMENT_MODE)
    ;
...
MACRO_CHAR_ENCLOSED
    : CALL_MACRO_CHAR { this.inputStream.LA(1) === '{'.charCodeAt(0) }?
...
ERROR
    : .
    { this.notifyListeners(new LexerNoViableAltException(this, this.inputStream, this.inputStream.index, undefined)); }
    -> channel(HIDDEN);
...
MACRO_INVALID
    : .
    { console.debug('invalid tokens'); this.notifyListeners(new LexerNoViableAltException(this, this.inputStream, this.inputStream.index, undefined)); }
...
MACRO_ARGUMENTS_MACRO
    : CALL_MACRO_CHAR
    {
        ...
        this.channel = UCLexer.MACRO_CHANNEL;
...
MACRO_ARGUMENTS_SYMBOL
    ...
    // >> MACRO_CHAR MACRO_SYMBOL OPEN_PARENS MACRO_SYMBOL COMMA MACRO_SYMBOL CLOSE_PARENS
    {
        ...
        this.inputStream.seek(this.inputStream.index - 1);

        let nest = 0;
        let isInQuote = false;

        while (true) {
            const charCode = this.inputStream.LA(1);
            switch (charCode) {
                case IntStream.EOF:
                    return;
                ...
                case 34: // '"'
                    // '\\'
                    if (this.inputStream.LA(-1) === 92) break;
...
MACRO_TEXT
    // Skip the first leading whitespace (so that MACRO_TEXT_WS can capture it)
    // TODO: Does the compiler strip the leading spaces?
    : ~[ \t\r\n] (~[\r\n]*? '\\' '\r'? '\n')* ~[\r\n]*
    -> channel(MACRO)
    ;
